...
In hidden mode, a single signing key is used by all paired devices. This allows all entities that have access to the private signing key to impersonate each other. This could allow one or more paired devices to be opaque to the MLS group, with inhibits the MLS goals of transparency of group membership. Thus, devices using the Paired MLS extension in hidden mode MUST be associated with the same group membership user identity, i.e., the paired devices may all belong to Alice but they should not belong to separate users Alice and Bob. 
