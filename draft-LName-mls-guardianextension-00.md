...
After the shared randomness is established the next step involves signing key assignment which determines if standard mode or hidden mode is used. In standard mode, the device initiating Paired MLS must register an additional signing key belonging to the target trusted device to their anchor. In hidden mode, the signing key of the initiating device is shared with the target trusted device: this MAY be done offline or out-of-band. 

After sharing randomness and linking to an anchor node, the devices are considered "paired" and either device may update on the other's behalf. When the active device issues an update to the group on behalf of the passive device, it will also issue a *Notify* notification message to the passive device to ratchet forward its group key. This ensures the passive device stays synchronized with the group epoch so it can process updates and commits made by other group members. This notification message is sent in place of a normal update to the paired device, i.e., such that the *Notify* message does not contain secret keying material.
